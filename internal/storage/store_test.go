...
func migrationsThroughVersion(t *testing.T, max int) fs.FS {
	...
	for _, name := range []string{
		"000001_initial.up.sql", "000002_indexes.up.sql", "000003_storage_contract.up.sql", "000004_storage_compatibility.up.sql", "000005_storage_safety.up.sql", "000006_approval_safety.up.sql", "000007_storage_recovery_safety.up.sql",
	} {
		var version int
...
func TestMigrationsUpgradeFromVersionOne(t *testing.T) {
	...
	if version != 7 {
		t.Fatalf("upgraded migration version = %d, want 7", version)
	}
	var count int
...
func TestMigrationsUpgradeFromVersionTwo(t *testing.T) {
	...
	if version != 7 {
		t.Fatalf("v2 upgrade migration version = %d, want 7", version)
	}
	var state string
