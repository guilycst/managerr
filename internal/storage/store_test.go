...
func migrationsThroughVersion(t *testing.T, max int) fs.FS {
	...
	for _, name := range []string{
		"000001_initial.up.sql", "000002_indexes.up.sql", "000003_storage_contract.up.sql", "000004_storage_compatibility.up.sql", "000005_storage_safety.up.sql",
	} {
		var version int
...
func TestMigrationsUpgradeFromVersionOne(t *testing.T) {
	...
	if version != 5 {
		t.Fatalf("upgraded migration version = %d, want 5", version)
	}
	var count int
...
func TestMigrationsUpgradeFromVersionTwo(t *testing.T) {
	...
	if version != 5 {
		t.Fatalf("v2 upgrade migration version = %d, want 5", version)
	}
	var state string
