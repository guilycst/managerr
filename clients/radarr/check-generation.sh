...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/mastarr-radarr-generate.XXXXXX")
...
if [ -e "$module_dir/generated/client.gen.go" ]; then
  echo "Radarr generated transport must remain module-internal" >&2
  exit 1
fi
...
if ! cmp -s "$tmp_dir/internal/generated/client.gen.go" "$module_dir/internal/generated/client.gen.go"; then
	echo "Radarr generated client is out of date" >&2
	exit 1
fi

echo "Radarr generation checks passed"
