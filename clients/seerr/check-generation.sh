...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/mastarr-seerr-generate.XXXXXX")
...
# is invoked from the pinned tools module below, so use an absolute temporary
# path while retaining every module-local generation setting.
...
if [ -e "$module_dir/generated/client.gen.go" ]; then
	echo "Seerr generated transport must remain module-internal" >&2
	exit 1
fi
...
if ! cmp -s "$tmp_dir/internal/generated/client.gen.go" "$module_dir/internal/generated/client.gen.go"; then
	echo "Seerr generated client is out of date" >&2
	exit 1
fi

echo "Seerr generation checks passed"
