...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/mastarr-sonarr-generate.XXXXXX")
...
if [ -e "$module_dir/generated/client.gen.go" ]; then
  echo "Sonarr generated transport must remain module-internal" >&2
  exit 1
fi
...
if ! cmp -s "$tmp_dir/internal/generated/client.gen.go" "$module_dir/internal/generated/client.gen.go"; then
  echo "Sonarr generated client is out of date" >&2
  exit 1
fi

echo "Sonarr generation checks passed"
